...
<div class="container pt-5">
    ...
    <footer class="footer">
              ...
              <?php
               ...
               $query6="SELECT SUM(JULY) as 'julysal' FROM monthtable";
               $res6=mysqli_query($con,$query6);
               $data6=mysqli_fetch_array($res6);

               $query7="SELECT SUM(AUG) as 'augsal' FROM monthtable";
               $res7=mysqli_query($con,$query7);
               $data7=mysqli_fetch_array($res7);

               $query8="SELECT SUM(SEPT) as 'septsal' FROM monthtable";
               $res8=mysqli_query($con,$query8);
               $data8=mysqli_fetch_array($res8);

               $query9="SELECT SUM(OCT) as 'octsal' FROM monthtable";
               $res9=mysqli_query($con,$query9);
               $data9=mysqli_fetch_array($res9);

               $query10="SELECT SUM(NOV) as 'novsal' FROM monthtable";
               $res10=mysqli_query($con,$query10);
               $data10=mysqli_fetch_array($res10);

               $query11="SELECT SUM(DECE) as 'decesal' FROM monthtable";
    ...
    <script type="text/javascript">
          ...
          var data = google.visualization.arrayToDataTable([
               ...
               ['JULY', <?php echo $data6['julysal']?>, "color: #040035" ],
               ['AUG' , <?php echo $data7['augsal']?>,  "color: #040035" ],
               ['SEPT', <?php echo $data8['septsal']?>, "color: #040035" ],
               ['OCT' , <?php echo $data9['octsal']?>,  "color: #040035" ],
               ['NOV' , <?php echo $data10['novsal']?>, "color: #040035" ],
               ['DECE', <?php echo $data11['decesal']?>,"color: #040035" ]
          ]);

        var options = {
          title: 'PAYOUT SUMMARY OF COMPANY (1 YEAR)',
          hAxis: {
                     title: 'MONTHS',
                 },
          vAxis: {
